...
def transform_srcProductSubcategory():
    ...
    revised.to_sql(
        f"stg_DimProductSubcategory", engine, if_exists="replace", index=False
    )
    return {"table(s) processed ": "Data imported successful"}
...
with DAG(
    ...
    with TaskGroup(
        "extract_dimProudcts_load", tooltip="Extract and load source data"
    ) as extract_load_src:
    ...
    with TaskGroup(
        "transform_src_product", tooltip="Transform and stage data"
    ) as transform_src_product:
        ...
        # define task order
        [
            transform_srcProduct,
            transform_srcProductSubcategory,
            transform_srcProductCategory,
        ]

    with TaskGroup(
        "load_product_model", tooltip="Final Product model"
    ) as load_product_model:
